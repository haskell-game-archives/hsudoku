...
spec = do
  describe "nodups" $ do
    ...
    it "should return true just for lists with no duplicates" $
      property $
        \xs -> nodups (xs :: [Int]) == (length xs == length (nub xs))

  describe "groupBy" $ do
    it "should return the empty list if given an empty list" $
      property $
        \n -> groupBy n ([] :: [Int]) `shouldBe` ([] :: [[Int]])

    it "should create the right amount of groups" $
      property $
        \xs ->
          all
            ( \n ->
    ...
    it "should create the right group sizes" $
      property $
        \xs ->
          all
            ( \n ->
  ...
  describe "single" $ do
    it "should be true if and only the list contains one element" $
      property $
        \xs -> single (xs :: [Int]) `shouldBe` (length xs == 1)

  describe "delete" $ do
    it "should not contain the deleted elements" $
      property $
